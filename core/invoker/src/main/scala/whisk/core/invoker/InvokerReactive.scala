...
class InvokerReactive(config: WhiskConfig,
  ...
  def cleanup() = {
    val cleaning = docker
      .ps(Seq("name" -> s"wsk${instance.toInt}_"))(TransactionId.invokerNanny)
      .flatMap { containers =>
  ...
  val containerFactory = SpiLoader
    ...
    .getContainerFactory(actorSystem, logging, config, dockerRunParameters)
    .createContainer _
  ...
  val ack = (tid: TransactionId,
    ...
    def send(res: Either[ActivationId, WhiskActivation],
  ...
  val childFactory = (f: ActorRefFactory) =>
    f.actorOf(
      ContainerProxy.props(containerFactory, ack, store, collectLogs, instance))
  ...
  val pool = actorSystem.actorOf(
    ContainerPool.props(childFactory,
                        maximumContainers,
                        maximumContainers,
                        activationFeed,
      ...
      .flatMap { msg =>
        ...
        WhiskAction
          .get(entityStore,
               actionid.id,
               actionid.rev,
               fromCache = actionid.rev != DocRevision.empty)
          .flatMap { action =>
            ...
            case t =>
              ...
              val causedBy =
                if (msg.causedBySequence)
                  Parameters("causedBy", "sequence".toJson)
                else Parameters()
