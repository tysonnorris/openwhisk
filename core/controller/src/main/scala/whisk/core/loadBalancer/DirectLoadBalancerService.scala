...
class DirectLoadBalancerServiceProvider extends LoadBalancerProvider {
  override def getLoadBalancer(config: WhiskConfig, instance: InstanceId)(
    ...
    new DirectLoadBalancerService(config, instance)
}

object DirectLoadBalancerServiceProvider
    extends SpiFactory[LoadBalancerProvider] {
  override def apply(dependencies: Dependencies): LoadBalancerProvider =
    new DirectLoadBalancerServiceProvider
}

class DirectLoadBalancerService(config: WhiskConfig, instance: InstanceId)(
  ...
  val dockerRunParameters = mutable.Map("--cap-drop" -> "NET_RAW",
                                        "--cap-drop" -> "NET_ADMIN",
  ...
  val containerFactory = SpiLoader
    ...
    .getContainerFactory(actorSystem,
                         logging,
                         config,
                         dockerRunParameters.toMap)
    .createContainer _
  ...
  val collectLogs = (tid: TransactionId,
                     container: Container,
  ...
  val childFactory = (f: ActorRefFactory) =>
    f.actorOf(
      ContainerProxy
        .props(containerFactory, ack, store, collectLogs, instance, 30.seconds))
  ...
  val feed: ActorRef = actorSystem.actorOf(
    DirectFeed.props(maxConcurrent, (r: Run) => { pool ! r }))
  ...
  val pool = actorSystem.actorOf(
    ContainerPool
      .props(childFactory,
             ...
             new ConcurrentPoolScheduler(maxConcurrent)),
    "poolactor"
  )
...
object DirectLoadBalancerService {
  def requiredProperties =
    Map(WhiskConfig.dockerRegistry -> "",
...
private class DirectFeed(concurrentLimit: Int, submit: Run => Unit)(
    implicit logging: Logging)
    extends Actor {
  val activations =
    mutable.Map[ActivationId, (ActorRef, ExecutableWhiskAction)]()
    ...
    case a: WhiskActivation => {
      activations.remove(a.activationId) match {
        ...
        case None =>
          logging.warn(
            this,
            s"received completion for unknown activation ${a.activationId}")
      }
    }
...
private object DirectFeed {
  def props(concurrentLimit: Int = 1, submit: Run => Unit)(
      implicit logging: Logging) =
    Props(new DirectFeed(concurrentLimit, submit))
}
private case class DirectLoadBalancerException(msg: String)
