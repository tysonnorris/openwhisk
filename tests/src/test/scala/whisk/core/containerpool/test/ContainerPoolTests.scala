...
class ContainerPoolTests
  ...
  val exec = CodeExecAsString(
    RuntimeManifest("actionKind", ImageName("testImage")),
    "testCode",
    None)
  val memoryLimit = 256.MB

  /** Creates a `Run` message */
  def createRunMessage(action: ExecutableWhiskAction,
  ...
  val runMessageDifferentNamespace =
    createRunMessage(action, differentInvocationNamespace)
  val runMessageDifferentEverything =
    createRunMessage(differentAction, differentInvocationNamespace)
  ...
  it should "create a container if it cannot find a matching container" in within(
    timeout) {
    val (containers, factory) = testContainers(2)
    val feed = TestProbe()
  ...
  it should "cache a container if there is still space in the pool" in within(
    timeout) {
    val (containers, factory) = testContainers(2)
    val feed = TestProbe()
  ...
  it should "use a prewarmed container and create a new one to fill its place" in within(
    timeout) {
    val (containers, factory) = testContainers(2)
    val feed = TestProbe()
  ...
  it should "not use a prewarmed container if it doesn't fit the kind" in within(
    timeout) {
    val (containers, factory) = testContainers(2)
    val feed = TestProbe()

    val alternativeExec =
      CodeExecAsString(RuntimeManifest("anotherKind", ImageName("testImage")),
                       "testCode",
                       None)
  ...
  it should "not use a prewarmed container if it doesn't fit memory wise" in within(
    timeout) {
    val (containers, factory) = testContainers(2)
    val feed = TestProbe()
    ...
    containers(0).send(pool,
                       NeedWork(preWarmedData(exec.kind, alternativeLimit)))
    pool ! runMessage
    containers(1).expectMsg(runMessage) // but container1 is used
  }
   ...
   * CONTAINER DELETION
   */
  it should "not reuse a container which is scheduled for deletion" in within(
    timeout) {
    val (containers, factory) = testContainers(2)
    val feed = TestProbe()
...
class ContainerPoolObjectTests extends FlatSpec with Matchers with MockFactory {
  ...
  val actionExec = CodeExecAsString(
    RuntimeManifest("actionKind", ImageName("testImage")),
    "testCode",
    None)
  ...
  def createAction(namespace: String = "actionNS",
  ...
  def preWarmedData(kind: String = "anyKind") =
    PreWarmedData(stub[Container], kind, 256.MB)
  ...
  it should "reuse an applicable warm container from idle pool with one container" in {
    ...
    ContainerPool.schedule(data.action.copy(),
                           data.invocationNamespace,
                           pool,
                           Map()) shouldBe Some('name, data)
  }
  ...
  it should "reuse an applicable warm container from idle pool with several applicable containers" in {
    ...
    ContainerPool.schedule(
      data.action.copy(),
      data.invocationNamespace,
      pool,
      Map()) should (be(Some('first, data)) or be(Some('second, data)))
  }
  ...
  it should "reuse an applicable warm container from idle pool with several different containers" in {
    ...
    val pool =
      Map('none -> noData(), 'pre -> preWarmedData(), 'warm -> matchingData)

    ContainerPool.schedule(matchingData.action.copy(),
                           matchingData.invocationNamespace,
                           pool,
                           Map()) shouldBe Some('warm, matchingData)
  }
  ...
  it should "not reuse a warm container with different invocation namespace" in {
    ...
    val differentNamespace =
      EntityName(data.invocationNamespace.asString + "butDifferent")
  ...
  it should "not reuse a warm container with different action name" in {
    val data = warmedData()
    val differentAction = data.action.copy(
      name = EntityName(data.action.name.asString + "butDifferent"))
    val pool = Map('warm -> data)

    data.action.name should not be differentAction.name
    ContainerPool.schedule(differentAction,
                           data.invocationNamespace,
                           pool,
                           Map()) shouldBe None
  }
  ...
  it should "not reuse a warm container with different action version" in {
    val data = warmedData()
    val differentAction =
      data.action.copy(version = data.action.version.upMajor)
    val pool = Map('warm -> data)
  ...
  it should "not provide a container from pool with one single free container with the same action and namespace" in {
    ...
    ContainerPool.remove(createAction(data.action.name + "butDifferent"),
                         data.invocationNamespace,
                         pool) shouldBe Some('warm)

    // different namespace, same action --> remove
    ContainerPool.remove(data.action, differentNamespace, pool) shouldBe Some(
      'warm)
  ...
  it should "provide oldest container from busy pool with multiple containers" in {
    ...
    val first = warmedData(namespace = commonNamespace,
                           lastUsed = Instant.ofEpochMilli(1))
    val second = warmedData(namespace = commonNamespace,
                            lastUsed = Instant.ofEpochMilli(2))
    val oldest = warmedData(namespace = commonNamespace,
                            lastUsed = Instant.ofEpochMilli(0))

    val pool = Map('first -> first, 'second -> second, 'oldest -> oldest)

    ContainerPool.remove(createAction(), standardNamespace, pool) shouldBe Some(
      'oldest)
  }
}
