...
class DockerClientWithFileAccessTestsIp
  ...
  def dockerClient(execResult: Future[String] =
                     Future.successful(ipInDockerInspect.asString),
    ...
    new DockerClientWithFileAccess()(global) {
      ...
      override def executeProcess(args: String*)(
          implicit ec: ExecutionContext) = execResult
      ...
      def publicIpAddressFromFile(id: ContainerId,
                                  network: String): Future[ContainerIp] =
  ...
  it should "throw NoSuchElementException if specified network is not in configuration file" in {
    ...
    a[NoSuchElementException] should be thrownBy await(
      dc.publicIpAddressFromFile(id, "foo network"))
  }
  ...
  it should "throw NoSuchElementException if specified network does not exist" in {
    ...
    a[NoSuchElementException] should be thrownBy await(
      dc.inspectIPAddress(id, "foo network"))
  }
}
...
class DockerClientWithFileAccessTestsLogs
  ...
  case class FixtureParam(file: File,
                          writer: FileWriter,
                          docker: DockerClientWithFileAccess) {
  ...
  override def withFixture(test: OneArgTest) = {
    val file = File.createTempFile(this.getClass.getName,
                                   test.name.replaceAll("[^a-zA-Z0-9.-]", "_"))
  ...
  it should "tolerate an empty log file" in { fixture =>
    ...
    val buffer =
      await(fixture.docker.rawContainerLogs(containerId, fromPos = 0))
    val logContent = new String(buffer.array,
                                buffer.arrayOffset,
                                buffer.position,
  ...
  it should "read a log file portion" in { fixture =>
    ...
    val buffer =
      await(fixture.docker.rawContainerLogs(containerId, fromPos = from))
    val logContent = new String(buffer.array,
                                buffer.arrayOffset,
                                buffer.position,
                                StandardCharsets.UTF_8)
  ...
  it should "provide an empty result on failure" in { fixture =>
    ...
    an[IOException] should be thrownBy await(
      fixture.docker.rawContainerLogs(containerId, fromPos = 0))
    stream should have size 0
  }
}
