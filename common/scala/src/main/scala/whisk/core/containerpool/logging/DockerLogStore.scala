...
object DockerLogStore extends LogStore{
    override def containerParameters = Map()//use defaults for docker logging of stdout/stderr

    override def logs(activation: WhiskActivation): Future[ActivationLogs] = {
        Future.successful(activation.logs)
    }

    override def collectLogs(container:Container, action: ExecutableWhiskAction)(implicit transid: TransactionId) = {
...
object DockerLogStoreProvider extends SpiFactory[LogStoreProvider] with LogStoreProvider {
    override def apply(dependencies: Dependencies) = DockerLogStoreProvider
